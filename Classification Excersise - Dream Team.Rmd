...
>>>>>>> 3d0b4cf80b6c4c8675692685b7856fa48c6c848a
```
...
evaluation(tree_cp001, test.df, "class")
...
```{r}
# Difference between cp 0.01 & 0.01995305
tree_i_pruned <- prune(tree_i, cp=0.01)
evaluation(tree_i_pruned, train.df, "class")
evaluation(tree_i_pruned, test.df, "class")
tree_i_pruned <- prune(tree_i, cp=0.01995305)
evaluation(tree_i_pruned, train.df, "class")
evaluation(tree_i_pruned, test.df, "class")

# Best Classification Tree
tree_best <- prune(tree_i,cp = 0.01995305) #prune tree
...
evaluation <- function(model, data, atype) {
  ...
  f = 2 * (precision * recall) / (precision + recall)
  cat(paste("Accuracy:\t", format(accuracy, digits=4), "\n",sep=" "))
  cat(paste("Precision:\t", format(precision, digits=4), "\n",sep=" "))
  cat(paste("Recall:\t\t", format(recall, digits=4), "\n",sep=" "))
  cat(paste("F-measure:\t", format(f, digits=4), "\n",sep=" "))
}

evaluation(tree_i, train.df, "class")
evaluation(tree, test.df, "class")
evaluation(tree_i, test.df, "class")
```
